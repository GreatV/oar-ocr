...
impl RecognitionBackend for NaviDcOcr {
    fn recognize(
        ...
        let prompt = recognition_prompt(task);
        // Doc-parser crops can be smaller than the 28px patch-merge factor;
        // pad/upscale them first (same treatment MinerU's backend applies).
        ...
        let out = self
            ...
            .ok_or_else(|| Error::InvalidInput {
                message: "NaviDC-OCR: no result returned".to_string(),
            })??;
        let raw = truncate_repetitive_content(&out, 10, 10, 10);
        Ok(recognition_postprocess(task, &raw))
    }
    ...
    fn recognize_batch(
        ...
        let prompts: Vec<&str> = tasks.iter().map(|task| recognition_prompt(*task)).collect();
        ...
        Ok(self
            ...
            .map(|(result, task)| {
                result.map(|out| {
                    let raw = truncate_repetitive_content(&out, 10, 10, 10);
                    recognition_postprocess(task, &raw)
                })
            })
            .collect())
    }
    ...
    fn needs_table_postprocess(&self) -> bool {
        true // Tables come back as OTSL; DocParser converts to HTML
    }
...
/// Map a doc-parser task to the official NaviDC-OCR prompt. Chart has no
/// dedicated prompt in the model card, so it falls back to plain text OCR.
fn recognition_prompt(task: RecognitionTask) -> &'static str {
    match task {
        RecognitionTask::Ocr | RecognitionTask::Chart => super::NaviDcTask::Text.prompt(),
        RecognitionTask::Table => super::NaviDcTask::Table.prompt(),
        RecognitionTask::Formula => super::NaviDcTask::Formula.prompt(),
    }
}

fn recognition_postprocess(task: RecognitionTask, raw: &str) -> String {
    match task {
        RecognitionTask::Formula => super::postprocess_formula(raw),
...
fn load_generation_config(path: impl AsRef<Path>) -> Option<NaviDcGenerationConfig> {
