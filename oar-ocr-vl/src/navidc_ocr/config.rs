...
/// Qwen2.5-VL vision tower configuration. Unlike the Qwen2-VL layout used by
/// MinerU2.5 (`embed_dim` + `mlp_ratio` + LayerNorm + plain MLP), this tower
/// spells the embedding width `hidden_size`, sizes the SwiGLU MLP through an
/// explicit `intermediate_size`, uses RMSNorm, and partitions attention into
/// `window_size` windows with periodic full-attention blocks.
#[derive(Debug, Clone, Deserialize)]
pub struct NaviDcVisionConfig {
    pub depth: usize,
    /// Per-patch embedding width (1280); doubles as the attention width.
    pub hidden_size: usize,
    /// Output width of the patch merger (1024, equals the text hidden size).
    pub out_hidden_size: usize,
    pub num_heads: usize,
    pub intermediate_size: usize,
    #[serde(default = "default_vision_hidden_act")]
    pub hidden_act: String,
    // NaviDC-OCR emits BOTH `in_chans` and `in_channels` with the same value;
    // a single aliased field trips serde's duplicate-field detection, so
    // capture each spelling separately and resolve through `in_channels()`.
    ...
    /// Full-attention window in input pixels (112); every other block attends
    /// within `window_size / (spatial_merge * patch)` merged-token windows.
    pub window_size: usize,
    /// Block indexes (e.g. [7, 15, 23, 31]) that attend over the whole image.
    #[serde(default)]
    ...
    pub fn head_dim(&self) -> Result<usize, Error> {
        if !self.hidden_size.is_multiple_of(self.num_heads) {
            return Err(Error::Config {
                message: format!(
                    "NaviDC-OCR: vision hidden_size {} not divisible by num_heads {}",
